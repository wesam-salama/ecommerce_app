...
class _StepoState extends State<Stepo> with TickerProviderStateMixin {
  ...
  void initProperties() {
    ...
    lowerBound = widget.lowerBound ?? 100;
    ...
    setState(() {
      if (widget.initialCounter == null) {
        if (_counter < upperBound) {
          if (_counter == 0) {
            _counter++;
            _size = widget.sizeList[_counter];
          } else if (_counter == widget.sizeList.length - 1) {
            _counter = 0;
            _size = widget.sizeList[_counter];
          } else {
            _counter++;
            _size = widget.sizeList[_counter];
            // print(widget.sizeList[_counter]);
          }
        }
      } else {
        _counter++;
